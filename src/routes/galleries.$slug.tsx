...
import { getCategory, categories } from "@/lib/galleries";
...
export const Route = createFileRoute("/galleries/$slug")({
  ...
  head: ({ loaderData }) => {
    if (!loaderData) {
      return { meta: [{ title: "Gallery — Gloria Reina" }, { name: "robots", content: "noindex" }] };
    }
    const c = loaderData.category;
    const title = `${c.name} — Gloria Reina`;
    return {
      meta: [
        { title },
    ...
    <div className="min-h-screen bg-background text-foreground">
      ...
      <main className="pt-32 md:pt-40">
        <div className="mx-auto max-w-[1600px] px-6 md:px-12">
          <Reveal>
            <Link to="/galleries" className="text-[11px] uppercase tracking-widest-plus text-muted-foreground hover:text-foreground transition">
              ← All galleries
            </Link>
          ...
          <div className="mt-24 flex items-center justify-between border-t border-border/60 pt-8 text-[11px] uppercase tracking-widest-plus text-muted-foreground">
            ...
            <div className="flex gap-6">
              {categories
                ...
                .slice(0, 2)
                .map((c) => (
                  <Link key={c.slug} to="/galleries/$slug" params={{ slug: c.slug }} className="hover:text-foreground transition">
                    {c.name} →
                  </Link>
                ))}
            </div>
  ...
  return (
    <div className="min-h-screen bg-background text-foreground flex items-center justify-center px-6">
      <div className="text-center">
        ...
        <h1 className="mt-4 font-serif text-4xl">Gallery not found</h1>
        <Link to="/galleries" className="mt-8 inline-block border-b border-foreground pb-1 text-[11px] uppercase tracking-widest-plus">
          Back to galleries
        </Link>
      </div>
    </div>
